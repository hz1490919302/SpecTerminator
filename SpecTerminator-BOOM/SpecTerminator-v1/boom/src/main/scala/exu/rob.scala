  ...
  )(implicit p: Parameters)  extends BoomBundle
{
  ...
  val setcyclerobidx = Input(Vec(2,UInt(32.W)))
  val fpsetcyclerobidx = Input(UInt(32.W)) 
}
  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  for (w <- 0 until coreWidth) {
    ...
    for(i <- 0 until 32){
      for(j <- 0 until 2){
         when((io.setcyclerobidx(j) === rob_uop(i).rob_idx || io.fpsetcyclerobidx === rob_uop(i).rob_idx) && rob_uop(i).validexecycle === 0.U){
            rob_uop(i).execycle := io.idle_cycles
            rob_uop(i).validexecycle := 1.U
         }
      }
    }          
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //clear
    //br_mask==0时，消除risk //rob_val(i) =/= 0.U
    for(i <- 0 until numRobRows) {

      val prs1_mask = Mux(rob_uop(i).lrs1_rtype === RT_FIX, io.risk_table(rob_uop(i).prs1), Mux(rob_uop(i).lrs1_rtype === RT_FLT, io.fp_risk_table(rob_uop(i).prs1),false.B))
      ...
      val M2ins = rob_uop(i).risk2 && !prs1_mask && !prs2_mask && !prs3_mask
      ...
      when( (rob_uop(i).risk1 || rob_uop(i).risk2) && (rob_uop(i).br_mask === 0.U || ( ((0 until 32).map{ k => rob_val(k) << rob_uop(k).br_tag }.reduce(_|_)&rob_uop(i).br_mask) === 0.U) ) && ((rob_uop(i).dst_rtype === RT_FIX && io.risk_table(rob_uop(i).pdst) === true.B) || (rob_uop(i).dst_rtype === RT_FLT && io.fp_risk_table(rob_uop(i).pdst) === true.B))){     // TODO
        ...
        when(rob_uop(i).debug_pc === 0x80001280L.U){
            printf(p"80001280-z\n")    
        }
        when(rob_uop(i).debug_pc === 0x80001292L.U){
            printf(p"80001292-z\n")
        }
        //当rob中的记录的推测性指令不在是推测的时，发出唤醒信号       //目前只是load唤醒，TODO
      }

      val load_can_clear = (0 until numStqEntries).map{ j => Mux(io.ldq_dep_mask(rob_uop(i).ldq_idx)(j) === 1.U ,!io.stq_valid(j) || io.stq_addr_valid(j)  , true.B)}.reduce(_&&_)
      //load解除污染
      val no_load_can_clear = (0 until numStqEntries).map{ j => Mux(io.ldq_dep_mask((rob_uop(i).ldq_idx+7.U)%8.U)(j) === 1.U ,!io.stq_valid(j) || io.stq_addr_valid(j) , true.B)}.reduce(_&&_)
      
      val prs1_wuran = Mux(rob_uop(i).lrs1_rtype === RT_FIX, io.st_risk_table(rob_uop(i).prs1), Mux(rob_uop(i).lrs1_rtype === RT_FLT, io.st_fp_risk_table(rob_uop(i).prs1),false.B))
      val prs2_wuran = Mux(rob_uop(i).lrs2_rtype === RT_FIX, io.st_risk_table(rob_uop(i).prs2), Mux(rob_uop(i).lrs2_rtype === RT_FLT, io.st_fp_risk_table(rob_uop(i).prs2),false.B))
      ...
      //非load解除污染  //rob_val(i) =/= 0.U &&
      when((rob_uop(i).risk3 || rob_uop(i).risk4) && ( (rob_uop(i).uses_ldq && load_can_clear) || (!rob_uop(i).uses_ldq && no_load_can_clear) ) && ((rob_uop(i).dst_rtype === RT_FIX && io.st_risk_table(rob_uop(i).pdst) === true.B) || (rob_uop(i).dst_rtype === RT_FLT && io.st_fp_risk_table(rob_uop(i).pdst) === true.B)) ){     // TODO
        ...
        when(rob_uop(i).debug_pc === 0x80001280L.U){
            printf(p"80001280-za\n")
        }
        when(rob_uop(i).debug_pc === 0x80001292L.U){
            printf(p"80001292-za\n")
        }
      }
    }
    ...
    for(i <- 0 until numIntPhysRegs+numFpPhysRegs) {
      when(clear_risk_table(i) =/= 0.U){
        clear_risk_table(i) := RegNext(0.U)
      }
      when(clear_st_risk_table(i) =/= 0.U){
        clear_st_risk_table(i) := RegNext(0.U)
      }
    }
...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
    ...
    for (clr <- io.lsu_clr_unsafe) {
      when (clr.valid && MatchBank(GetBankIdx(clr.bits))) {
   	...
   	//fudiv_interference为0，无干扰
    io.st_fudiv_interference := (0 until numRobRows).map{ i => rob_val(i) && rob_uop(i).fu_code === 16.U && rob_bsy(i) && ((rob_uop(i).dst_rtype === RT_FIX && io.st_risk_table(rob_uop(i).pdst) === false.B) || (rob_uop(i).dst_rtype === RT_FLT && io.st_fp_risk_table(rob_uop(i).pdst) === false.B))  }.reduce(_||_) =/= 0.U  
    ...
    val ttt = (0 until numRobRows).map{ i => rob_val(i) && rob_uop(i).debug_pc === 0x800011ceL.U }.reduce(_||_) =/= 0.U

          when(ttt){
             for(j <- 0 until numRobRows){
               when(rob_val(j)){
                when(rob_uop(j).debug_pc === 0x8000114eL.U){
                    printf(p" 1cycles=${io.idle_cycles} ")
                    printf(p" fucode=${rob_uop(j).fu_code === 128.U} ")
                    printf(p" val=${rob_val(j)} ")
                    printf(p" bsy=${rob_bsy(j)} ")
                    printf(p" FLT=${rob_uop(j).dst_rtype === RT_FLT} ")
                    printf(p" st_fp=${io.st_fp_risk_table(rob_uop(j).pdst) === false.B} ")
                    printf(p" fp=${io.fp_risk_table(rob_uop(j).pdst) === false.B} ")
                    printf(p" st=${io.st_risk_table(rob_uop(j).pdst) === false.B} ")
                    printf(p" risk=${io.risk_table(rob_uop(j).pdst) === false.B} \n")
                }
                when(rob_uop(j).debug_pc === 0x80001152L.U){
                    printf(p" 2cycles=${io.idle_cycles} ")
                    printf(p" fucode=${rob_uop(j).fu_code === 128.U} ")
                    printf(p" val=${rob_val(j)} ")
                    printf(p" bsy=${rob_bsy(j)} ")
                    printf(p" FLT=${rob_uop(j).dst_rtype === RT_FLT} ")
...
when(rob_uop(j).debug_pc === 0x80001156L.U){
                    printf(p" 3cycles=${io.idle_cycles} ")
                    printf(p" fucode=${rob_uop(j).fu_code === 128.U} ")
                    printf(p" val=${rob_val(j)} ")
                    printf(p" bsy=${rob_bsy(j)} ")
                    printf(p" FLT=${rob_uop(j).dst_rtype === RT_FLT} ")
...
when(rob_uop(j).debug_pc === 0x8000115aL.U){
                    printf(p" 4cycles=${io.idle_cycles} ")
                    printf(p" fucode=${rob_uop(j).fu_code === 128.U} ")
                    printf(p" val=${rob_val(j)} ")
                    printf(p" bsy=${rob_bsy(j)} ")
                    printf(p" FLT=${rob_uop(j).dst_rtype === RT_FLT} ")
...
when(rob_uop(j).debug_pc === 0x8000115eL.U){
                    printf(p" 5cycles=${io.idle_cycles} ")
                    printf(p" fucode=${rob_uop(j).fu_code === 128.U} ")
                    printf(p" val=${rob_val(j)} ")
                    printf(p" bsy=${rob_bsy(j)} ")
                    printf(p" FLT=${rob_uop(j).dst_rtype === RT_FLT} ")
...
when(rob_uop(j).debug_pc === 0x80001162L.U){
                    printf(p" 6cycles=${io.idle_cycles} ")
                    printf(p" fucode=${rob_uop(j).fu_code === 128.U} ")
                    printf(p" val=${rob_val(j)} ")
                    printf(p" bsy=${rob_bsy(j)} ")
                    printf(p" FLT=${rob_uop(j).dst_rtype === RT_FLT} ")
    ...
    when (rbk_row) {  //回滚时，commit为false
      ...
      rob_uop(com_idx).risk3 := 0.U
      rob_uop(com_idx).risk4 := 0.U
      rob_predicated1(com_idx)   := false.B
      wb_valid(com_idx)   := false.B
      wb_robidx(com_idx)   := com_idx
      val index_pdst=Mux(rob_uop(com_idx).dst_rtype === RT_FLT,rob_uop(com_idx).pdst+numIntPhysRegs.U,rob_uop(com_idx).pdst)
      when(io.risk_table(rob_uop(com_idx).pdst) && rob_uop(com_idx).dst_rtype===RT_FIX){
        clear_risk_table(index_pdst) := 1.U
      }
when(io.fp_risk_table(rob_uop(com_idx).pdst) && rob_uop(com_idx).dst_rtype===RT_FLT){
        clear_risk_table(index_pdst) := 1.U
      }
when(io.st_risk_table(rob_uop(com_idx).pdst) && rob_uop(com_idx).dst_rtype===RT_FIX){
        clear_st_risk_table(index_pdst) := 1.U
      }
when(io.st_fp_risk_table(rob_uop(com_idx).pdst) && rob_uop(com_idx).dst_rtype===RT_FLT){
        clear_st_risk_table(index_pdst) := 1.U
      }
      ...
      when (RegNext(exception_thrown)) {
        for (i <- 0 until numRobRows) {
          ...
          wb_robidx(i)   := i.U
             ...
             val index_pdst=Mux(rob_uop(i).dst_rtype === RT_FLT,rob_uop(i).pdst+numIntPhysRegs.U,rob_uop(i).pdst)
      when(io.risk_table(rob_uop(i).pdst) && rob_uop(i).dst_rtype===RT_FIX){
        clear_risk_table(index_pdst) := 1.U
      }
when(io.fp_risk_table(rob_uop(i).pdst) && rob_uop(i).dst_rtype===RT_FLT){
        clear_risk_table(index_pdst) := 1.U
      }
when(io.st_risk_table(rob_uop(i).pdst) && rob_uop(i).dst_rtype===RT_FIX){
        clear_st_risk_table(index_pdst) := 1.U
      }
when(io.st_fp_risk_table(rob_uop(i).pdst) && rob_uop(i).dst_rtype===RT_FLT){
        clear_st_risk_table(index_pdst) := 1.U
      }
        }
      }
...
when(io.st_fp_risk_table(rob_uop(i).pdst) && rob_uop(i).dst_rtype===RT_FLT){
    ...
    when (will_commit(w)) {
      rob_val(rob_head) := false.B
        ...
        rob_uop(rob_head).risk1 := 0.U
        rob_uop(rob_head).risk2 := 0.U
        rob_uop(rob_head).risk3 := 0.U
        rob_uop(rob_head).risk4 := 0.U

       val index_pdst=Mux(rob_uop(rob_head).dst_rtype === RT_FLT,rob_uop(rob_head).pdst+numIntPhysRegs.U,rob_uop(rob_head).pdst)
      when(io.risk_table(rob_uop(rob_head).pdst) && rob_uop(rob_head).dst_rtype===RT_FIX){
        clear_risk_table(index_pdst) := 1.U
      }
when(io.fp_risk_table(rob_uop(rob_head).pdst) && rob_uop(rob_head).dst_rtype===RT_FLT){
        clear_risk_table(index_pdst) := 1.U
      }
when(io.st_risk_table(rob_uop(rob_head).pdst) && rob_uop(rob_head).dst_rtype===RT_FIX){
        clear_st_risk_table(index_pdst) := 1.U
      }
when(io.st_fp_risk_table(rob_uop(rob_head).pdst) && rob_uop(rob_head).dst_rtype===RT_FLT){
        clear_st_risk_table(index_pdst) := 1.U
      }
    }
