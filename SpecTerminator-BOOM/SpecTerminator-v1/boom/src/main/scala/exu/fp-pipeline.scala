...
class FpPipeline(implicit p: Parameters) extends BoomModule with tile.HasFPUParameters
{
  ...
  val io = IO(new Bundle {
    ...
    val risk_table_interference = Input(Vec(numIntPhysRegs,Bool()))
    val fp_risk_table_interference = Input(Vec(numFpPhysRegs,Bool()))
    val st_risk_table_interference = Input(Vec(numIntPhysRegs,Bool()))
    val st_fp_risk_table_interference = Input(Vec(numFpPhysRegs,Bool()))
	
    val fudiv_interference = Input(Bool())
    val fufdiv_interference = Input(Bool())
    val st_fudiv_interference = Input(Bool())
    val st_fufdiv_interference = Input(Bool())
    
    val fpsetcyclerobidx = Output(UInt(32.W))
  })
  ...
  issue_unit.io.risk_table <> io.risk_table
  issue_unit.io.fp_risk_table <> io.fp_risk_table
  issue_unit.io.st_risk_table <> io.st_risk_table
  issue_unit.io.st_fp_risk_table <> io.st_fp_risk_table

  issue_unit.io.risk_table_interference <> io.risk_table_interference
  issue_unit.io.fp_risk_table_interference <> io.fp_risk_table_interference
  issue_unit.io.st_risk_table_interference <> io.st_risk_table_interference
  issue_unit.io.st_fp_risk_table_interference <> io.st_fp_risk_table_interference
  
  issue_unit.io.fudiv_interference <> io.fudiv_interference
  issue_unit.io.fufdiv_interference <> io.fufdiv_interference
  ...
  for (i <- 0 until issue_unit.issueWidth) {
    ...
    io.fpsetcyclerobidx := Mux(iss_valids(0),iss_uops(0).rob_idx,33.U)
