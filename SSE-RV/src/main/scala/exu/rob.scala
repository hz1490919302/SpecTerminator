  ...
  )(implicit p: Parameters) extends BoomModule
{
  ...
  for (w <- 0 until coreWidth) {
    ...
    val clear_st_risk_table = RegInit(VecInit(Seq.fill(numIntPhysRegs+numFpPhysRegs){0.U(1.W)}))   
    ...
    for(i <- 0 until numRobRows) {
      ...
      when( (rob_uop(i).risk1 || rob_uop(i).risk2) && IsOlder(rob_uop_mask.rob_idx, rob_pnr_idx, rob_head_idx) && ((rob_uop(i).dst_rtype === RT_FIX && io.risk_table(rob_uop(i).pdst) === true.B) || (rob_uop(i).dst_rtype === RT_FLT && io.fp_risk_table(rob_uop(i).pdst) === true.B))){     // TODO
...
//                  "FP instruction writing back exc bits is overriding an existing exception.")
    ...
    //When fudiv_interference is 0, it means no risk
    ...
    when (rbk_row) {
      ...
      when((io.risk_table(rob_uop(com_idx).pdst) || io.st_risk_table(rob_uop(com_idx).pdst)) && rob_uop(com_idx).dst_rtype===RT_FIX){
        clear_risk_table(index_pdst) := 1.U
      }
      when((io.fp_risk_table(rob_uop(com_idx).pdst) || io.st_fp_risk_table(rob_uop(com_idx).pdst)) && rob_uop(com_idx).dst_rtype===RT_FLT){
        clear_risk_table(index_pdst) := 1.U
      }
    }
    ...
    if (enableCommitMapTable) {
      when (RegNext(exception_thrown)) {
        for (i <- 0 until numRobRows) {
          ...
          rob_uop(i).risk4 := 0.U
          val index_pdst=Mux(rob_uop(i).dst_rtype === RT_FLT,rob_uop(i).pdst+numIntPhysRegs.U,rob_uop(i).pdst)
          when((io.risk_table(rob_uop(i).pdst) || io.st_risk_table(rob_uop(i).pdst)) && rob_uop(i).dst_rtype===RT_FIX){
             clear_risk_table(index_pdst) := 1.U
          }
          when((io.fp_risk_table(rob_uop(i).pdst) || io.st_fp_risk_table(rob_uop(i).pdst)) && rob_uop(i).dst_rtype===RT_FLT){
             clear_risk_table(index_pdst) := 1.U
          }
        }
      }
    ...
    when (will_commit(w)) {
      rob_val(rob_head) := false.B
        ...
        when((io.risk_table(rob_uop(rob_head).pdst) || io.st_risk_table(rob_uop(rob_head).pdst)) && rob_uop(rob_head).dst_rtype===RT_FIX){
          clear_risk_table(index_pdst) := 1.U
        }
        when((io.fp_risk_table(rob_uop(rob_head).pdst) || io.st_fp_risk_table(rob_uop(rob_head).pdst)) && rob_uop(rob_head).dst_rtype===RT_FLT){
          clear_risk_table(index_pdst) := 1.U
        }
    }
