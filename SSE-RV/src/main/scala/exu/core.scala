...
//      ("I$ blocked",                        () => icache_blocked),
  ...
  for (i <- 0 until coreWidth) {
    ...
    // Block the load instruction of a store with an unresolved address in front of it
    ...
    //If there is a store whose address is not resolved in stq when a load is dispatched, the destination register of the load is identified as risk (modified by risk_table)
    when( false.B && dis_uops(i).uses_ldq && dis_uops(i).pdst =/= 0.U && io.lsu.storeaddr_not_get ){
      when(dis_fire(i) ) {
     ...
     .elsewhen( false.B && dis_uops(i).pdst =/= 0.U && (prs1_wuran || prs2_wuran || prs3_wuran) ) { //&& !dis_uops(i).uses_ldq
        when(dis_fire(i)) {
          when(dis_uops(i).dst_rtype === RT_FLT) {
            st_fp_risk_table(dis_uops(i).pdst) := true.B
	  ...
	  when(false.B && dis_uops(i).uses_ldq && dis_uops(i).pdst =/= 0.U && (prs1_wuran || prs2_wuran || prs3_wuran) ) {
	     when(dis_fire(i)) { 
               when(dis_uops(i).dst_rtype === RT_FLT) {
	  ...
	  when( false.B && dis_uops(i).pdst =/= 0.U && (prs1_wuran_interference || prs2_wuran_interference) ) {
	      when(dis_fire(i)) { 
                when(dis_uops(i).dst_rtype === RT_FLT) {
                  st_fp_risk_table_interference(dis_uops(i).pdst) := true.B
                }.elsewhen(dis_uops(i).dst_rtype === RT_FIX) {
    ...
    val maskvalid = false.B // ((0 until 32).map{ k => rob.io.rob_val(k) << rob.io.rob_uop(k).br_tag }.reduce(_|_) & disuopsmask.br_mask) =/= 0.U
    ...
    when(!IsOlder(disuopsmask.rob_idx, rob.io.rob_pnr_idx, rob.io.rob_head_idx) && dis_uops(i).uses_ldq && dis_uops(i).pdst =/= 0.U && !(prs1_wuran_mask || prs2_wuran_mask || prs3_wuran_mask) ){
      when(dis_fire(i) ) {
        when(dis_uops(i).dst_rtype === RT_FLT){
     ...
     .elsewhen(!IsOlder(disuopsmask.rob_idx, rob.io.rob_pnr_idx, rob.io.rob_head_idx) && dis_uops(i).pdst =/= 0.U && (prs1_wuran_mask || prs2_wuran_mask || prs3_wuran_mask)){ //
        when(dis_fire(i) ) {
          when(dis_uops(i).dst_rtype === RT_FLT){
            fp_risk_table(dis_uops(i).pdst) := true.B
    	  ...
    	  when(false.B && disuopsmask.br_mask =/= 0.U && maskvalid && dis_uops(i).uses_ldq && dis_uops(i).pdst =/= 0.U && (prs1_wuran_mask || prs2_wuran_mask || prs3_wuran_mask) ){
	    when(dis_fire(i) ) {
              when(dis_uops(i).dst_rtype === RT_FLT){
	  ...
	  when(false.B && dis_uops(i).br_mask =/= 0.U && maskvalid && dis_uops(i).pdst =/= 0.U && (prs1_wuran_mask_interference || prs2_wuran_mask_interference) ) {
	     when(dis_fire(i) ) { 
               when(dis_uops(i).dst_rtype === RT_FLT){
	             fp_risk_table_interference(dis_uops(i).pdst) := true.B
	       } .elsewhen(dis_uops(i).dst_rtype === RT_FIX) {
