...
class LSUDMemIO(implicit p: Parameters, edge: TLEdgeOut) extends BoomBundle()(p)
{
  ...
  val idle_cycles = Output(UInt(32.W))   //For logging cycles when debugging
  ...
  with rocket.HasL1HellaCacheParameters
{
 ...
 // Prioritize emptying the store queue when it is almost full
  ...
  for (w <- 0 until memWidth) {
    ...
    dtlb.io.store_risk_tlb(w)       := false.B // (will_fire_stad_incoming(w) || will_fire_sta_incoming(w) || will_fire_sta_retry(w)) && (io.core.risk_table(exe_tlb_uop(w).prs1) || io.core.st_risk_table(exe_tlb_uop(w).prs1)) && exe_tlb_uop(w).uses_stq && exe_tlb_uop(w).lrs1_rtype === RT_FIX 
   // dtlb.io.store_risk_tlb(w)       := (will_fire_stad_incoming(w) || will_fire_sta_incoming(w) || will_fire_sta_retry(w)) && (io.core.risk_table(exe_tlb_uop(w).prs1) ) && exe_tlb_uop(w).uses_stq && exe_tlb_uop(w).lrs1_rtype === RT_FIX 
    ...
    dtlb.io.load_risk(w)             := false.B // (will_fire_load_incoming(w) || will_fire_load_retry(w)) && exe_tlb_uop(w).uses_ldq && (( (prs1_risk || prs2_risk || prs3_risk) && pdst_risk ) || ( (prs1_risk_st || prs2_risk_st || prs3_risk_st) && pdst_risk_st ))
    ...
    dtlb.io.all_load_risk(w)         := false.B // (will_fire_load_incoming(w) || will_fire_load_retry(w)) && exe_tlb_uop(w).uses_ldq && (pdst_risk || pdst_risk_st)
 }
